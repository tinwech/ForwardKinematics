...
int main() {
    ...
    acclaim::Motion walking(acclaim_folder / "walk.amc", std::make_unique<acclaim::Skeleton>(*skeleton));
    acclaim::Motion shooting(acclaim_folder / "shoot.amc", std::make_unique<acclaim::Skeleton>(*skeleton));
    acclaim::Motion shoot_copy(acclaim_folder / "shoot_copy.amc", std::make_unique<acclaim::Skeleton>(*skeleton));
    acclaim::Motion running(acclaim_folder / "running.amc", std::make_unique<acclaim::Skeleton>(*skeleton));
    acclaim::Motion punch_kick(acclaim_folder / "punch_kick.amc", std::make_unique<acclaim::Skeleton>(*skeleton));

    acclaim::Motion walk_run(walking, running);
    acclaim::Motion shoot_punch(shoot_copy, punch_kick);

    shooting.getSkeleton()->setBoneColor(Eigen::Vector4f(0.12f, 0.28f, 0.53f, 0.0f));
    walk_run.getSkeleton()->setBoneColor(Eigen::Vector4f(0.12f, 0.53f, 0.28f, 0.0f));
    shoot_punch.getSkeleton()->setBoneColor(Eigen::Vector4f(0.53f, 0.12f, 0.28f, 0.0f));
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (isTimeWarping)
            ...
            totalFrames = walk_run.getFrameNum();
        else if (isBonus2)
            totalFrames = shoot_punch.getFrameNum();
        else
            totalFrames = walking.getFrameNum();
        ...
        if (isTimeWarping) {
            ...
            walk_run.setBoneTransform(currentFrame);
        } else if (isBonus2) {
            shoot_punch.setBoneTransform(currentFrame);
        } else {
            walking.setBoneTransform(currentFrame);
        }
        ...
        if (isTimeWarping) {
            ...
            walk_run.getSkeleton()->render(&shadowProgram);
        } else if (isBonus2) {
            shoot_punch.getSkeleton()->render(&shadowProgram);
        } else {
            walking.getSkeleton()->render(&shadowProgram);
        }
        ...
        if (isTimeWarping) {
            ...
            walk_run.getSkeleton()->render(&renderProgram);
        } else if (isBonus2) {
            shoot_punch.getSkeleton()->render(&renderProgram);
        } else {
            walking.getSkeleton()->render(&renderProgram);
        }
        // 3. Render the skybox .
...
void mainPanel(int* frame, int maxFrame) {
    ...
    if (ImGui::Begin(ICON_CODE " Frame Control")) {
        ...
        ImGui::SameLine();
        if (ImGui::Button("walk_run")) {
            isBonus1 ^= true;
            isBonus2 = false;
            *frame = 0;
        }
        ImGui::SameLine();
        if (ImGui::Button("shoot_punch")) {
            isBonus2 ^= true;
            isBonus1 = false;
            *frame = 0;
        }
